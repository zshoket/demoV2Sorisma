				...
				link: function (scope, element, attrs) {
					...
					if (attrs.template) {
						scope.$watch('template', function () {
							if (scope.template) {
								createMatrix('<div class="risk-matrix"><div class="left-label">Eintrittswahrscheinlichkeit</div><div class="bottom-label">Schadenshoehe</div><div style="position:absolute;width:100%;height:100%;"><div risk-matrix-item ng-repeat="item in items" class="risk-matrix-item" style="position:absolute;">' + scope.template + '</div></div></div>');
							}
						});
					}
					else {
						createMatrix('<div class="risk-matrix"><div class="left-label">Eintrittswahrscheinlichkeit</div><div class="bottom-label">Schadenshoehe</div><div style="position:absolute;width:100%;height:100%;"><div risk-matrix-item ng-repeat="item in items" class="risk-matrix-item" style="position:absolute;"><div class="closed"><span ng-bind="item.Id"></span></div><div class="open"><div ng-bind="\'Likelihood: \'+item.RiskLikelihood"></div><div ng-bind="\'Impact: \'+item.RiskImpact"></div></div></div></div></div>');
					}
				}
			};
		}])
